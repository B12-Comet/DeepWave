...
class TemporalDataset(object):
    def __init__(self, name: str):
        ...
        # Examine the data loading section to ensure no training-test data mixing occurs.
        ...
        valid_set = set(tuple(x) for x in valid_data)
        ...
        try:
            ...
            self.time_diffs = torch.from_numpy(pickle.load(inp_f)).to(device).float()
    ...
    def time_AUC(self, model: TKBCModel, split: str, n_queries: int = -1):
        ...
        truth, scores = model.get_auc(test.to(device))
